...
.envs: .conda .channels
	export PATH=${PBASE}/bin/bin:${PATH} && conda create -y --prefix=${PBASE}/bin/envs/rna samtools=1.7 igvtools=2.3.93 alfred=0.1.7 fastqc=0.11.7 bcftools=1.7 star=2.6.0c bedtools=2.27.1 && conda install -y --prefix=${PBASE}/bin/envs/rna -c conda-forge ncurses=5.9 && touch .envs
